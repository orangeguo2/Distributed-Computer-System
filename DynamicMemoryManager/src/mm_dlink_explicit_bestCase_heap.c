...
//	fprintf(stderr, "  \n freep is %p\n", freep);
	...
	FreePointer *startHere = NULL;
	size_t smalbigSize = 0;
	FreePointer *p = freep->next;
	for (;; p = p->next) {
...
//
//				fprintf(stderr, "  \n  p prev is  %p\n", p->prev);
				if (curHeader->size < smalbigSize || smalbigSize == 0) {
					startHere = p;
					smalbigSize = curHeader->size;
				}
...
//	fprintf(stderr, "\n go here \n ");
	Header *startHeader = (Header*) startHere - 1;
	Header *nextHeader = startHeader;
	if (startHeader->size == nunits) { /* exactly */
		startHere->prev->next = startHere->next;
		startHere->next->prev = startHere->prev;

		Header* tail = startHeader + startHeader->size - 1;
		startHeader->size = startHeader->size | 1;
		tail->size = startHeader->size;

	} else { /* sstartHerelit allocate tail end */
		/* adjust the size to sstartHerelit the block */
//		fprintf(stderr, "  \n  block size is bigger than \n");
		startHeader->size -= nunits;
		Header* tail = startHeader + startHeader->size - 1;
		tail->size = startHeader->size;

		/* find the address to return */
		nextHeader = startHeader + startHeader->size;
	...
	freep = startHere->prev; /* move the head */
//
